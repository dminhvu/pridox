...
const DocumentDisplay = ({
  ...
  return (
    <div
      ...
      className="mb-3 border-b border-border text-sm"
    >
      <div className="relative flex">
        <a
          className={
            "flex rounded-lg font-bold " +
        ...
        >
          ...
          <p className="my-auto ml-2 truncate break-all text-base">
            {document.semantic_identifier || document.document_id}
          </p>
        </a>
      </div>
      <div className="mt-1 flex flex-wrap gap-x-2 text-xs">
        <div className="flex rounded bg-hover px-1 py-0.5">
        ...
        <div
          ...
          className="flex cursor-pointer select-none rounded bg-hover px-1 py-0.5 hover:bg-hover-light"
        >
      ...
      <p className="break-words pb-3 pl-1 pt-2">
...
export function Explorer({
  ...
  useEffect(() => {
    ...
    if (query && query.trim() !== "") {
      router.replace(`/data-sources?query=${encodeURIComponent(query)}`);
  ...
  return (
    <div>
      ...
      <div className="justify-center py-2">
        <div className="flex w-fit items-center rounded-full bg-slate-50 px-4 py-2 focus-within:border-accent">
          <SearchIcon size={20} />
          <textarea
            autoFocus
            className="ml-2 h-6 w-80 flex-grow resize-none overflow-hidden whitespace-normal bg-transparent text-sm placeholder-gray-400 outline-none"
            role="textarea"
            aria-multiline
      ...
      {!query && (
        <div className="mt-3 flex text-emphasis">
