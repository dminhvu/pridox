...
export const ChatSidebar = ({
  ...
  function SidebarNavigation() {
    ...
    return (
      <nav className="flex w-full max-w-[279px] flex-col items-start gap-4 whitespace-nowrap px-3 pb-3 text-base leading-4 tracking-wider">
        {NavItems.map((item) => (
          <NavItem
            key={item.label}
            icon={item.icon}
    ...
    >
      ...
      <article className="mt-auto flex max-w-[231px] flex-col rounded-2xl bg-[linear-gradient(15deg,#7C3AED_26.3%,#6C1AF7_86.4%)] px-4 pb-7 text-center text-sm text-white">
        ...
        <p className="mt-4 font-medium leading-6">
          Get your AI Project to another level and start doing more with Pridox
          PRO!
        </p>
