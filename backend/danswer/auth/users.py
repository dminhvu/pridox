...
from danswer.configs.app_configs import CLERK_PEM_PUBLIC_KEY
from danswer.configs.constants import AuthType
from danswer.db.auth import get_access_token_db
from danswer.db.auth import get_user_count
from danswer.db.auth import get_user_db
from danswer.db.engine import get_session, get_sqlalchemy_async_engine
...
async def double_check_user(
    ...
    if optional:
        ...
        if(token):
            try :
                data = jwt.decode(token, CLERK_PEM_PUBLIC_KEY, algorithms=["RS256"])
                async with AsyncSession(get_sqlalchemy_async_engine()) as asession:
                    ...
                    if not user:
                        user = await user_db.create({'email':data['email'], 'hashed_password': '', 'is_verified': True, 'role': UserRole.ADMIN})
                    return user
            except:
                raise HTTPException(
